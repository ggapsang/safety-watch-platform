...
| `zones.py` | 판정 기하를 이미지로 되돌린 그림 — 위험구역·도달범위 |
| `geometry.py` | 호모그래피(DLT·역투영)·전방 스윕·통로 판정 (순수 함수) |
...
| `tests.py` | 브로커·플랫폼·카메라 없이 도는 자체 검증 (140개) |
...
| `PUBLISH_LIVE` | `false` | 화면용 오버레이를 `aivision/live` 로도 낼지(종합 현황) |
| `OVERLAY_ZONES` | `true` | 라이브에 위험구역·도달범위를 함께 실을지 |
| `TICK_HZ` | `10` | 판정 루프 주기 |
...
4. **임계값 조정.** 바꾸면 곧바로 반영됩니다(컨테이너 재시작 불필요). 미리보기에서
   위험구역과 도달범위가 늘고 주는 것을 보며 맞추세요 — 숫자만 보고 맞추는 것보다 빠릅니다.
...
판정이 쓰는 기하를 **그대로** 이미지에 되돌려 그립니다. 그림이 판정과 다른 숫자를 쓰면
구역 밖에 선 사람에게 알람이 울리는 화면이 되므로, 통로 기하는 `judge.corridor_of()`,
반경은 `judge.reach_radius()` 하나에서만 나옵니다.

| 그림 | 무엇 | 어떻게 |
|---|---|---|
| **위험구역** | AMR 진행 방향 통로. 판정이 '주의' 를 매기는 그 직사각형 | 길이 `max(최소길이, r_amr + 속도·T_warn)`, 반폭 `corridor_half_w + r_h` |
| **도달범위** | 사람이 임박 전망 시간 안에 갈 수 있는 바닥 원 `R_h(τ)=r_h+σ+v_h,max·τ` | 24각형으로 쪼개 투영(원근을 지나면 타원) |

둘이 겹치는 것이 곧 임박에 가까워지는 것입니다.

### 세 갈래로 내보냅니다

1. **모듈 화면** — `GET /api/overlay/{camera_id}`. 원근이 살아 있는 다각형을 영상
   (MJPEG 중계 `GET /api/cameras/{id}/stream`) 위에 겹쳐 그립니다. `PUBLISH_LIVE` 와
   **무관하게 늘 열려 있습니다** — 자기 화면에서 보는 것과 남에게 보내는 것은 다른 결정입니다.
2. **종합 현황 (계약 채널)** — `aivision/live/{camera_id}` 의 `boxes` 에 구역의 **축정렬
   사각형**을 함께 싣습니다. 0~1 정규화에 사람이 읽을 라벨(`위험구역 AMR#3`)이라 코어가
   **오늘 그대로** 그립니다. 코어 수정이 필요 없습니다. 화면에서 '종합 현황에도 보내기' 를
   켜면 됩니다(기본 꺼짐).
3. **확장 채널** — 같은 payload 의 `shapes` 필드에 다각형과 미터 단위 원본을 함께 보냅니다.
   계약 밖 키라 코어는 무시하고, 나중에 원근이 살아 있는 그림이 필요해지면 그때 읽으면 됩니다.

```json
{"camera_id": 1, "module_id": "collision", "ts": "...",
 "boxes": [ {"x1":0.31,"y1":0.44,"x2":0.72,"y2":0.93,"label":"위험구역 AMR#2 · 임박"} ],
 "shapes": [
   {"kind":"corridor","track_id":2,"level":"imminent","label":"위험구역 AMR#2 · 임박",
    "points":[[0.31,0.62],[0.55,0.44],[0.72,0.51],[0.40,0.93]],
    "world":{"length_m":3.17,"half_width_m":1.15,"speed_ms":0.53,"heading_deg":0.0}},
   {"kind":"reach","track_id":2,"level":"","label":"도달범위 사람#2",
    "points":[[...]],
    "world":{"radius_m":1.95,"horizon_sec":1.0,"center":[6.0,5.0]}}]}
```

> **내보내는 것은 우리가 만든 그림뿐입니다** — 위험구역과 도달범위. 사람·AMR 박스는
> 싣지 않습니다. 그것은 객체감지·카메라 메타데이터가 이미 내고 있고, 코어가 발행자별로
> 나눠 들고 있다가 합쳐 그리므로(`useLiveBoxes`) 되실으면 같은 물체 위에 사각형이 두 번
> 그려집니다. 계약 문서의 **'자기가 만든 것만 내십시오'** 입니다.
>
> 다른 발행자의 라이브를 끌 필요는 없습니다. 예전에는 코어가 카메라별로 박스를 통째로
> 갈아 끼워 두 그림이 번갈아 깜빡였고 그때는 끄는 것이 회피책이었지만, 코어가 고쳐졌습니다.
>
> 모듈 자기 화면은 영향받지 않습니다. 그쪽은 사람·AMR 까지 한 벌을 그대로 그립니다 —
> 자기 화면에서 보는 것과 남에게 보내는 것은 다른 결정입니다.

## 검증

```
python tests.py         # 브로커·플랫폼·카메라 없이 돈다 (140개)
```
