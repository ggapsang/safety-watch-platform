...
> **켜기 전에 알아야 할 것.** 코어 대시보드는 카메라별로 **마지막 라이브 메시지의 박스를
> 통째로 교체**합니다(`web/src/lib/hooks.ts`의 `useLiveBoxes`). 같은 카메라에 객체감지
> 모듈도 라이브를 내고 있으면 두 발행자의 그림이 번갈아 보입니다. 이 모듈의 그림이
> 원본 박스까지 포함한 상위집합이므로, 구역까지 보려면 **그쪽 라이브를 끄는** 편이 낫습니다.
> (코어가 발행자별로 합쳐 그리게 하려면 코어 수정이 필요합니다 — 플랫폼 팀의 결정입니다.)
