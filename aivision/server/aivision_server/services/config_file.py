...
async def replace(session: AsyncSession, data: dict,
    ...
    다만 탐지 항목은 지우지 않는다. 항목 하나에 사고 이력이 딸려 있는데, JSON 에서 몇
    글자 지운 것이 그것을 날리는 일이 되어서는 안 된다. 빠진 항목이 있으면 거부하고
    무엇이 걸리는지 알려 준다 — 정말 지울 거라면 `DELETE /api/solutions/{code}` 가
    몇 건이 사라지는지 알려 주고 확인을 받는다.
    ...
    if dropped:
        raise ConfigError(
            ...
            + " — 항목에 딸린 사고 이력이 함께 사라질 수 있습니다. "
              "DELETE /api/solutions/<코드> 로 몇 건이 걸리는지 확인하고 지우세요.")
